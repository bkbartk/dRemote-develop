...
#Region "XML"
            Private Function DecryptCompleteFile() As String
                ...
                If strCons <> "" Then
                    ...
                    Try
                        strDecr = Security.Crypt.DecryptStringFromBytes_Aes(strCons, PWAES)
                        If IsNothing(strDecr) OrElse Not strDecr.Contains("<?xml version=""1.0"" encoding=""utf-8""?>") Then
            ...
            Private Function Authenticate(ByVal Value As String, ByVal CompareToOriginalValue As Boolean, Optional ByVal rootInfo As Root.Info = Nothing) As Boolean
                ...
                If CompareToOriginalValue Then
                    Dim AesValue As String = Nothing
                    Do Until (Not IsNothing(AesValue) AndAlso AesValue <> Value) OrElse Security.Crypt.Decrypt(Value, pW) <> Value
                        pW = Tools.Misc.PasswordDialog(passwordName, False)
                        PWAES = pW
                        If pW = "" Then
                            Return False
                        End If
                        AesValue = Security.Crypt.DecryptStringFromBytes_Aes(Value, PWAES)
                    Loop
                    ...
                    '    Loop
                    'ElseIf EncryptionType = "AES" Then
                    '    Do Until Security.Crypt.DecryptStringFromBytes_Aes(Value, PWAES) <> Value
                    '        pW = Tools.Misc.PasswordDialog(passwordName, False)
                ...
                Else
                    ...
                    ElseIf EncryptionType = "AES" Then
                        If Not String.IsNullOrEmpty(My.Settings.MasterPassword) Then
                            ...
                            End If
                        End If
                        If PWAES = App.Info.General.EncryptionKeyAES Then
                            Do Until Security.Crypt.DecryptStringFromBytes_Aes(Value, PWAES) = "ThisIsProtected"
                                PWAES = Tools.Misc.PasswordDialog(passwordName, False)
